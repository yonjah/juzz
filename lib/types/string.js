...
const debug       = require('debug')('Juzz:String');
...
const StringExample = {
    joiStringExample(desc, rules) {
        ...
        if (guid) {
            ...
            if (max !== undefined && res.length > max ) {
                res = res.substring(0, res[max - 1] === '/' ? max - 1 : max);
            }
            else if (min !== undefined) {
                while (min > res.length) {
                    res += '/' + encodeURI(chance.string({ min:1, max:10, pool: 'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789!@$%^&*()' }));
                }
            }
        }
        else if (email) {
            ...
            if (min !== undefined && min > res.length) {
            ...
            if (length === undefined) {
                const tMin = Math.min(Math.max(this.options.stringMin, min || 0),  max !== undefined ? max : Infinity);
                const tMax = Math.max(Math.min(this.options.stringMax, max !== undefined ? max : Infinity), min || 0);
                length = chance.integer({ min: tMin, max: tMax });
                debug('Length', { length, tMin, tMax, min, max });
            }

            if (length < 0) {
                debug('Length is ', length, 'returning empty string');
                return '';
            }
