...
const StringExample = {
	joiStringExample (desc, rules) {
		let res,
			{length, min, max, guid, alphanum, regex, ip, email, lowercase, uppercase, hex, trim, dataUri, base64, isoDate, uri, hostname, normalize, token} = rules;
		const {flags = {}} = desc;
		...
		if (guid) {
			...
			debug('email', email);
			const {min: rmin, max: rmax} = this.joiDescRulesObject(desc, false);
			let keepStart = true,
				tldWhitelist = email.tldWhitelist,
				atoms = chance.integer({min: email.minDomainAtoms || 0, max: email.minDomainAtoms ? email.minDomainAtoms + 3 : 3}),
				domain;
			...
			if (rmin && rmin > res.length) {
				let part = chance.string({min: rmin - res.length, max: rmin - res.length, pool: 'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789'});
				res = keepStart ? res + part : part + res ;
			}

			if (rmax && res.length > rmax) {
				res  = keepStart ? res.substring(0, rmax) : res.substring(res.length - max);
			...
			if (min === 0 && desc.invalids && desc.invalids.indexOf('') >= 0) {
				min = 1;
			}

			if (!length) {
				length = chance.integer({min, max});
				debug('Length', length, min, max);
			}
			...
			if (regex && !regex.invert) {
				...
				if (rmin) {
					...
					if (exp[0] !== '^') {
						exp = `${extendedPattern}${rmin}${exp}`;
						truncEnd = false;
					} else if (exp[exp.length - 1] !== '$') {
						exp = `${exp}${extendedPattern}${rmin}`;
					}
				...
				if (rmax && res.length > rmax) {
					res = res.substring(truncEnd ? 0 : res.length - rmax, truncEnd ? rmax : res.length);
				}
			} else if (hex) {
	...
	genStringGuid (desc, rules) {
		...
		let {guid, regex} = rules;
		let {min, max} = this.joiDescRulesObject(desc, false);
		let res,
			version;
		if (guid.version) {
	...
	genStringIsoDate (desc/*, rules*/) {
		let {length, max} = this.joiDescRulesObject(desc, false);
	...
	getRegexDef(regex) {
		let exp = regex.toString().substring(1),
			expEnd = exp.lastIndexOf('/'),
			flags = exp.substring(expEnd + 1, exp.length);
