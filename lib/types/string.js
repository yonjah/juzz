...
const StringExample = {
    joiStringExample(desc, rules) {
        ...
        if (guid) {
            ...
            if (length === undefined) {
                length = chance.integer({ min: min !== undefined ? min : this.options.stringMin, max: max !== undefined ? max : this.options.stringMax });
                debug('Length', length, min !== undefined ? min : this.options.stringMin, max !== undefined ? max : this.options.stringMax);
            }
