...
const StringExample = {
    joiStringExample(desc, rules) {
        ...
        if (guid) {
            ...
            if (rmin && rmin > res.length) {
                const part = chance.string({ min: rmin - res.length, max: rmin - res.length, pool: 'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789' });
    ...
    genStringGuid(desc, rules) {
        ...
        const { guid, regex } = rules;
        const { min, max } = this.joiDescRulesObject(desc, false);
        let res;
    ...
    genStringIsoDate(desc/*, rules*/) {
        ...
        const { length, max } = this.joiDescRulesObject(desc, false);
