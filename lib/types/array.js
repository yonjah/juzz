...
const ArrayExample = {
    joiArrayExample(desc, rules) {
        ...
        let { length, min, max, unique, has } = rules;
        const { flags = {} } = desc;
        ...
        if (has) {
            has.forEach((item) => {
                ...
                const mainDesc = allowedItems.find((mItem) => mItem.type === item.type);
                res.push(mainDesc ? this.genExample(DescHelpers.merge(mainDesc, item)) : this.genExample(item));
            });
        }
