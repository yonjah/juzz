...
const ExampleTools = Object.assign({
	...
	testStrictProperty (key, obj) {
		if (obj && knownKeys[key]) {
			const dif = _.difference(Object.keys(obj), knownKeys[key]);
			if (dif.length) {
				debug.enabled && dif.forEach((key) => debug('unknown key', key, obj[key]));
				throw new Error(`${key} has unknown keys ${dif}`);
			}
		}
	},
...
function CreateExample (schema, op) {
